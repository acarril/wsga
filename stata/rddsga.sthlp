...
{pstd}
{cmd:rddsga} conducts binary subgroup analysis in regression discontinuity (RD) settings using inverse propensity score weighting (IPSW).
...
By default {cmd:rddsga} uses the percentile method: the empirical p-value is the share of bootstrap draws with |coef| >= |estimate|, and CIs are the 2.5th/97.5th percentiles of the bootstrap distribution.
...
{phang2}{cmd:. rddsga Y Z, balance(X1) sgroup(G) bwidth(10) dibalance}{p_end}

{pstd}Reduced form with IPW balancing on X1 and X2, 200 bootstrap replications{p_end}
{phang2}{cmd:. rddsga Y Z X1 X2, sgroup(G) bwidth(10) reducedform bsreps(200)}{p_end}

{pstd}Fuzzy RD via 2SLS, saving IPW weights{p_end}
{phang2}{cmd:. rddsga Y Z X1 X2, sgroup(G) bwidth(6) ivregress fuzzy(T) ipsweight(wt)}{p_end}

{pstd}Sharp RD without IPW{p_end}
{phang2}{cmd:. rddsga Y Z, sgroup(G) bwidth(10) reducedform noipsw nobootstrap}{p_end}
...
Please report issues at {browse "https://github.com/acarril/rddsga/issues"}.
